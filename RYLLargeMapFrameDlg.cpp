...
//	lpTabButton->m_pTabButton->SetTooltip( &m_TooltipManager, CRYLStringTable::m_strString[ 2245 ], 1852, TOOLTIP_TAB ) ;
...
VOID	CRYLLargeMapFrameDlg::GMUIPaint( INT xx, INT yy ) 
{
...
*/
	...
	m_pMyPosSprite->m_pSprite->Render(lpD3DDevice, pSelfCreature->GetDirection() + FLOAT_PHI, 0x80);
	...
	{
		// 석상전 존일때는 석상
		if( (SERVER_ID::STONE_WAR1 <= CRYLNetworkData::Instance()->m_dwPresentZone && CRYLNetworkData::Instance()->m_dwPresentZone <= SERVER_ID::STONE_WAR3) && GetStatue())
		{
			CRYLGameData* pGame = CRYLGameData::Instance();
