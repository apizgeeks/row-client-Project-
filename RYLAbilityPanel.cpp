...
HRESULT		CRYLAbilityPanel::Update( BOOL &bClick, BOOL &bEdge )
{
	...
	if ( m_dwEraseResult & MB_YES )
	{
		...
		SendPacket::CharSkillErase( g_GameSession, CRYLNetworkData::Instance()->m_dwMyChrID,
									...
									m_dwProcessAbilityPosIndex, &m_lpStatusRetrainItem->m_lpItemBase->GetPos() ) ;
...
*/
	...
	sprintf( strString, "%d / %d", pGame->m_csStatus.GetUseAbilityPoint(),  pGame->m_csStatus.GetAbilityPoint() ) ;
	...
	case RYLABILITYPANEL::ERASE_BUTTON :
		{
			...
			if(lpItem)
			{
				g_pAbilityPanel->m_lpStatusRetrainItem = lpItem;
