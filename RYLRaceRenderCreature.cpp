							   ...
							   short wPosX, short wPosY, unsigned long ulBackColor, unsigned long ulHPColor)
{
	...
	{	
		// 체력 게이햨E바
		float fHPRate = ((float)pData->m_lCurrHP / (float)pData->m_ulMaxHP) * 68.0f;
		RECT rcRect = { wPosX - 34, wPosY + 30, wPosX + 34, wPosY + 34 };
		...
		pGame->RenderRect(lpD3DDevice, rcRect.left, rcRect.top, rcRect.right, rcRect.bottom, ulHPColor, 0xFF);

		SetRect(&rcRect, wPosX - 34, wPosY + 30, wPosX - 34 + fHPRate, wPosY + 34);
		...
		pGame->RenderRect(lpD3DDevice, rcRect.left, rcRect.top, rcRect.right, rcRect.bottom, ulBackColor, 0xFF);

		m_lpHPGauge->m_nPutX = wPosX - 41;
		m_lpHPGauge->m_nPutY = wPosY + 29;
