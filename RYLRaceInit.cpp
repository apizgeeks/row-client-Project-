...
BOOL CRYLRaceBase::InitChrSelect(void)
{
...
//	m_ucEnableRide = 0;
	for (int i = 0; i < MAX_CHAR_SLOT; i++) // Minotaurs 캐릭 처음 불러올
	{
		if (pGame->m_scLoginChar[i].CID)
		{
			...
			{
				strShape[j] = g_ItemMgr.GetEquipName(pGame->m_scLoginChar[i].Equip[j]);
			}
			...
			RYLCreatureManager::Instance()->AddCreature( pGame->m_scLoginChar[i].CID, 
				...
				pGame->m_scLoginChar[i].Race, pGame->m_vecCharSelectPos[i], pGame->m_fCharSelectDir[i], false );
...
{
	CRYLGameData*	pGame = CRYLGameData::Instance() ; // Minotaurs 게임 초기화
	DestroyAllObject();
	RYLCreatureManager::Instance()->ClearCreatureManager();

	if (!strcmp(pGame->m_csStatus.m_Info.Name, ""))
	{
		...
		pGame->m_csStatus.m_Skill.SSlot[0].SKILLINFO.wSkill = 0x8102;
	...
	if (0 == static_cast<unsigned short>(pGame->m_csStatus.m_Info.HP))
	{
		// 죽은후 Login 했을때
		CSceneManager::m_pFullSceneEffect->SetLockFlag( FULLSCENE_MOTIONBLUR,false ) ;
		CSceneManager::m_pFullSceneEffect->SetLockFlag(FULLSCENE_SATURATION2,false);
...
#define ADD_UI_TEXTURE(szTextureName) \
	...
	ADD_UI_TEXTURE("bsmith01.dds");		
	ADD_UI_TEXTURE("mmap02.dds");		
