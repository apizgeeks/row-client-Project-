...
CRYLBattleGroundGameStatusDlg::CRYLBattleGroundGameStatusDlg()
{
    ...
    CRYLSpriteList::Instance()->CreateGMImage( 517, 89, &pImageDIB );
	pImageDIB->ClearbyColor( 0x00000000 );
    // 휴먼 얼굴
    ...
    // 시계(?)
    SetRect( &rt,  0, 68,128,146 );
    pImageDIB->DrawRect( &rt,201,  0, CRYLSpriteList::Instance()->FindSprite( ERYLSPRITENAME::RYL_BATTLEGROUND_GAME_STATUS_256x256 ) );
    // 아칸 얼굴
    ...
    SetLeft( ( CGMUIScreen::GetInstance()->GetWidth() - GetWidth() ) / 2 - 260 );                                
	SetTop( 15 );
    SetWidth( 516 );
	SetHeight( 78 );
    ...
    m_ptHumanScore.x= GetLeft() + 43;
    m_ptHumanScore.y= GetTop() + 33;

    m_ptTime.x      = GetLeft() + 212;
    m_ptTime.y      = GetTop() + 26;

    m_ptAkhanScore.x= GetLeft() + 370;
    m_ptAkhanScore.y= GetTop() + 33;

    for(int i = 0 ; i < 3 ; ++i )
    {
        m_nTargetHumanScoreNumber[ i ] = 0;
        m_nTargetAkhanScoreNumber[ i ] = 0;

        m_queTargetHumanScoreNumber[ i ].reserve( 9 );
        m_queTargetHumanScoreNumber[ i ].resize( 9 );
        m_queTargetAkhanScoreNumber[ i ].reserve( 9 );
        m_queTargetAkhanScoreNumber[ i ].resize( 9 );
    }
