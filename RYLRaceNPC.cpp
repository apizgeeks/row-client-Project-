...
#endif
{
	...
	switch (m_dwKindPopup)
	{
		...
		case POPUP_A_COMBATANTTRAINER:
			...
			if ( (pGame->m_csStatus.m_Info.Level >= 10 && pGame->m_csStatus.m_Info.Class == CClass::Fighter && m_dwKindPopup == POPUP_FIGHTERTRAINER) ||
				(pGame->m_csStatus.m_Info.Level >= 10 && pGame->m_csStatus.m_Info.Class == CClass::Combatant && m_dwKindPopup == POPUP_A_COMBATANTTRAINER) )
			{
		...
		case POPUP_A_OPPICATERTRAINER:
			...
			if ( (pGame->m_csStatus.m_Info.Level >= 10 && pGame->m_csStatus.m_Info.Class == CClass::Rogue && m_dwKindPopup == POPUP_ROGUETRAINER) ||
				(pGame->m_csStatus.m_Info.Level >= 10 && pGame->m_csStatus.m_Info.Class == CClass::Officiator && m_dwKindPopup == POPUP_A_OPPICATERTRAINER) )
			{
				pmAddMenu.m_dwPopupKind = POPUPMENU_JOBCHANGE;
				strcpy(pmAddMenu.m_strPopupStr, CRYLStringTable::m_strString[9]);
				m_lstPopupMenu.push_back(pmAddMenu);
							...
							if (lpJobNode)
							{
								...
								else
								{
									...
									if (true == CRYLNetworkData::Instance()->UseContents(GameRYL::STONE_BATTLE))
									{
										...
										{
											if((SERVER_ID::STONE_WAR1 <= lpJobNode->m_dwZoneNumber && lpJobNode->m_dwZoneNumber <= SERVER_ID::STONE_WAR3) && pSelfCreature->GetRealmWarFlag() == Creature::WAR_OFF)
											{
												CRYLMessageBox *lpMessageBox = new CRYLMessageBox;
							...
							if (pZone)
							{
								...
								else
								{
									...
									if (true == CRYLNetworkData::Instance()->UseContents(GameRYL::STONE_BATTLE))
									{
										if (CRYLGameTimeManager::Instance()->IsRealmWarReadyTime() || CRYLGameTimeManager::Instance()->IsRealmWarTime())
										{						
											if((SERVER_ID::STONE_WAR1 <= pZone->m_wZoneNumber && pZone->m_wZoneNumber <= SERVER_ID::STONE_WAR3) && pSelfCreature->GetRealmWarFlag() == Creature::WAR_OFF)
											{
												CRYLMessageBox *lpMessageBox = new CRYLMessageBox;
												lpMessageBox->SetWordBreak(true);
												lpMessageBox->Create(CRYLStringTable::m_strString[3581], MB_YES | MB_NO | MB_EXIT);
