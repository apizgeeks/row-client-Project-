...
*/
	...
	if ( bAble )
	{
		if ( ( m_dwFlag & MB_NO ) || ( m_dwFlag & MB_CANCEL ) )
		{
			if ( m_pNoButton )
			{
				if ( ( m_bEnableBack && GetAsyncKeyState(VK_BACK) ) || m_pNoButton->GetIsClick() )
//				if ( m_pNoButton->GetIsClick() )
			...
			if ( m_pCancelButton )
			{
				if ( ( m_bEnableBack && GetAsyncKeyState(VK_BACK)) || m_pCancelButton->GetIsClick() )
//				if ( m_pCancelButton->GetIsClick() )
		...
		if ( ( m_dwFlag & MB_YES ) || ( m_dwFlag & MB_CONFIRM ) || ( m_dwFlag & MB_ARRAYCONFIRM ) )
		{
			if ( m_pConfirmButton )
			{
				if ( ( m_bEnableEnter && GetAsyncKeyState(VK_RETURN)) || ( m_pConfirmButton->GetIsClick() && g_DeviceInput.GetIsLeftMouseUp() ) )
				{
