...
LONG __stdcall UserUnhandledExceptionFilterEx(PEXCEPTION_POINTERS pExceptionInfo) 
{
...
VOID	CClientMain::QueryRegScreenValue() 
{
	...
	RegCreateKeyEx(HKEY_LOCAL_MACHINE, "Software\\ROW\\MP-Client", 0, NULL, 
...
BOOL CClientMain::Init(HINSTANCE hInstance, INT iWidth, INT iHeight )
{
	...
	wc.lpszClassName = "Return Of Warrior";
...
**/
	...
	m_hWnd = CreateWindow(	"Return Of Warrior",					// class
							"Return Of Warrior",					// caption
							dwWinStyle,							// style 
							0, 0, 
							dwTmpWidth,
	...
	if(!pSystemInfo->IsMinimum())
	{
		...
		if(bSave)
		{
			if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, strReg, 0, KEY_ALL_ACCESS, &hMPClientReg)==ERROR_SUCCESS)
			{
				...
				{
					MessageBox(NULL, "Not Render Option Save", "Return Of Warrior", MB_OK);
				}
				
				RegCloseKey(hMPClientReg);
			}

			MessageBox(NULL, CRYLStringTable::m_strString[3801], "Return Of Warrior", MB_OK);
		}				
	}	
...
void CClientMain::Update(void)
{
	...
	if (g_DeviceInput.KeyDown(VK_F12) ) 
	{
	...
	else if (g_DeviceInput.KeyDown(VK_SNAPSHOT) ) 
	{
		ProcessScreenshot();
	}
	...
	if ( m_pRYLGameData->m_dwExecuteExplorerConfirm & MB_CONFIRM || m_pRYLGameData->m_dwExecuteExplorerConfirm & MB_EXIT )
	{
		...
		char szPath[1024] = { 0, };
		strcpy(szPath, "http://www.returnofwarrior.com");
...
void CClientMain::ProcessScreenshot()
{
	...
	{
		sprintf(strTemp, "%s_ROW_%04d_%02d_%02d_%02d_%02d(%d).%s", strText, 
			1900+ptm->tm_year, ptm->tm_mon+1, ptm->tm_mday, ptm->tm_hour+1, ptm->tm_min+1,
			dwNumScreenShot, strExt);
	}
